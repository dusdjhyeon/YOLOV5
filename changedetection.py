...
class ChangeDetection:
    ...
    def send(self, save_dir, image):
        ...
        today = datetime.now()
        save_path = os.path.join('C:/Users/dusdj/project', save_dir, 'detected', str(today.year), str(today.month), str(today.day))

        pathlib.Path(save_path).mkdir(parents=True, exist_ok=True)

        full_path = Path(save_path) / '{0}-{1}-{2}-{3}.jpg'.format(today.hour, today.minute,today.second, today.microsecond)
